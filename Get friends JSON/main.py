...
class VkFriends():
	...
	def base_info(self, ids):
		...
		r = requests.get(self.request_url('users.get', 'user_ids=%s&fields=photo' % (','.join(map(str, ids))), access_token=True)).json()
		if 'error' in r.keys():
	...
	def friends(self, id):
		...
		r = requests.get(self.request_url('friends.get',
				'user_id=%s&fields=uid,first_name,last_name,photo' % id, access_token=True)).json()['response']
	...
	def common_friends(self):
		...
		for i in VkFriends.parts(list(self.all_friends.keys())):
			data = requests.get(self.request_url('execute.getMutual',
							'source=%s&targets=%s' % (self.my_id, VkFriends.make_targets(i)), access_token=True)).json()
			if 'error' in data and 'error_code' in data['error'] and data['error']['error_code'] == 6: time.sleep(2)
			elif 'error' in data: respect = 0
			else:
				'''print(data)'''
				r = data['response']
